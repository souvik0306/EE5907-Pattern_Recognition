...
# Initialize weights and biases with simple random initialization
...
# Perform a single backward pass and update weights
...
# Compute predictions and classification accuracy after update
predictions = (a2 > 0.5).astype(int).flatten()
accuracy = np.mean(predictions == T)

# Print final accuracy
print(f'Final Classification Accuracy after one update: {accuracy * 100:.2f}%')
...
def plot_decision_boundary(X, T, forward_pass, W1, b1, W2, b2):
    ...
    plt.title(f'Decision Boundary of MLP using Backprop with Accuracy = {accuracy*100:.2f}%')
    plt.show()
